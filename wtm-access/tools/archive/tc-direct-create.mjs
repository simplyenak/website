// Navigate directly to create product URL
...
async function main() {
  ...
  for (const product of PRODUCTS) {
    ...
    // Fill price
    const priceField = visibleFields.find(f => f.type === 'text' && (f.placeholder?.toLowerCase().includes('price') || f.placeholder?.toLowerCase().includes('amount') || f.id?.includes('price')));
    ...
    if (buttons.length > 0) {
      ...
      if (btn && btn.rect && btn.rect.width > 0) {
    ...
    if (match) {
      ...
      console.log(`  Product not created, URL: ${newUrl}`);
      // Screenshot to debug
      await cdp.send('Page.captureScreenshot', { format: 'png' });
