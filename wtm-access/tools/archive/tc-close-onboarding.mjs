// ThriveCart product creation - close onboarding first
...
async function dismissOnboarding(cdp) {
  // Try clicking the "Close" link specifically
  await evalJs(cdp, `
    (() => {
      // Find Close link in onboarding modal
      const closeLinks = Array.from(document.querySelectorAll('a, span'));
      const closeLink = closeLinks.find(l => l.textContent.trim().toLowerCase() === 'close');
      if (closeLink) {
        closeLink.click();
        return 'clicked close link';
      }
      
      // Try finding any close button
      const closeBtns = Array.from(document.querySelectorAll('button'));
      const closeBtn = closeBtns.find(b => b.textContent.trim().toLowerCase() === 'close');
      if (closeBtn) {
        closeBtn.click();
  ...
  const check = await evalJs(cdp, `
    (() => {
      // Look for onboarding modal indicators
      const text = document.body.innerText;
      return {
        hasOnboarding: text.includes('Get Started'),
        hasClose: text.includes('Close')
      };
    })()
  `);
  console.log(`  Onboarding status: ${JSON.stringify(check)}`);
}
