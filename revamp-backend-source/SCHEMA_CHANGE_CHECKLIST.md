...
for table in "${TABLES[@]}"; do
  if PGPASSWORD=***REMOVED*** psql -h localhost -U directus -d payload-local -c "\d $table" &>/dev/null; then
    echo "✅ $table exists"
  else
    echo "❌ $table MISSING"
  fi
...
PGPASSWORD=***REMOVED*** psql -h localhost -U directus -d payload-local -c "
