...
### Why Blocks Over Flat Fields

Blocks provide:
- **Editorial flexibility**: Content sections can be reordered, added, or removed
- **Type safety**: Each block has a defined schema with TypeScript types
- **Reusability**: Same block types used across multiple collections
- **Maintainability**: Clear separation of content sections

### Block Table Naming Convention

Payload auto-generates table names from collection slug and block slug:

```
{collection_slug}_blocks_{block_slug}           -- Main block table
{collection_slug}_blocks_{block_slug}_{field}   -- Nested array sub-table
```

**Example:**
```typescript
// Collection: home_page, Block slug: heroBlock
// Creates: home_page_blocks_hero_block
{
  name: 'heroSection',
  type: 'blocks',
  blocks: [{
    slug: 'heroBlock',  // ← Determines table name
    fields: [
      { name: 'title', type: 'text' },
      { name: 'badges', type: 'array', fields: [{ name: 'text', type: 'text' }] }
      // Creates: home_page_blocks_hero_block_badges
    ],
  }],
}
```

**CRITICAL**: Block slugs must be lowercase with no special characters. Changing a block slug after data exists requires manual table renaming.

### Shared Block Patterns

Create reusable block schemas across collections:

```typescript
// Shared hero block used by 7+ collections
const heroBlock = {
  slug: 'heroBlock',
  fields: [
    { name: 'title', type: 'text' },
...
### Block Migration Pattern

When converting flat fields to blocks:

1. **Create block tables** with correct naming convention
2. **Migrate data** from flat columns to block tables
3. **Set `block_name`** to the block slug (Payload uses this internally)
4. **Set `_path`** to the blocks field name (e.g., 'heroSection')
5. **Keep old columns** until admin UI is verified working

```sql
-- Migration example
INSERT INTO home_page_blocks_hero_block 
  (_parent_id, _order, _path, block_name, title, subtitle)
SELECT id, 0, 'heroSection', 'heroBlock', hero_title, hero_subtitle 
FROM home_page;
```

### Nested Arrays in Blocks

Arrays inside blocks create sub-tables:
...
```typescript
{
  ...
  blocks: [{
    ...
    fields: [{
      name: 'stats',
      type: 'array',  // Creates: home_page_blocks_stats_block_stats
      fields: [
...
**Avoid `richText` in blocks** unless you need full lexical editor. Use `textarea` for simple text content to avoid JSONB column complexity.

### Reference Data Collections

Small, centralized collections used as relationship targets:
...
export const DietaryOptions: CollectionConfig = {
  ...
  admin: {
    group: 'Reference Data',  // Group in admin sidebar
    useAsTitle: 'name',
  },
  fields: [
...
**Reference collections in Simply Enak:**
- `dietary_options` (12 items)
- `travel_types` (4 items)
- `specialty_experiences` (4 items)
- `locations` (2 items)
...
11. **Block Table Naming**: `{collection}_blocks_{block_slug}` — slug determines table name
12. **Block `block_name` Column**: Payload expects `block_name` (not `blockType`) in DB
13. **Version Tables**: Collections with `versions: { drafts: true }` need `_collection_v` rows
14. **Locked Documents**: `payload_locked_documents_rels` needs `{collection}_id` for every collection
15. **Enum Types**: PostgreSQL enums must be updated when adding select options to existing collections
16. **richText in Blocks**: Creates JSONB columns — use `textarea` for simple text to avoid complexity
