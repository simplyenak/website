...
export default buildConfig({
  telemetry: false,
  // i18n: TODO - fix format for Payload 3.x
  admin: {
    user: Users.slug,
    importMap: {
  ...
  plugins: [
    ...
    s3Storage({
      collections: {
        media: {
          ...
          // Serve directly from public S3 URL — avoids needing credentials just to display images
          generateFileURL: ({ filename }) =>
            `${process.env.S3_ENDPOINT || 'https://s3.nl-ams.scw.cloud'}/${process.env.S3_BUCKET || 'se-website-images'}/${filename}`,
        },
      },
      bucket: process.env.S3_BUCKET || 'se-website-images',
      config: {
        credentials: {
    ...
    // SEO Plugin
    seoPlugin({
      collections: ['tours', 'stories', 'landing_pages'],
